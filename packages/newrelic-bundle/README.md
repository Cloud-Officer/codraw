Codraw NewRelic Bundle
=====================

[![Latest Version](https://img.shields.io/github/release/codraw/DrawNewRelicBundle.svg?style=flat-square)](https://github.com/Cloud-Officer/codraw-newrelic-bundle/releases)
[![Total Downloads](https://img.shields.io/packagist/dt/codraw/newrelic-bundle.svg?style=flat-square)](https://packagist.org/packages/codraw/newrelic-bundle)
...
```bash
$ composer require codraw/newrelic-bundle
```
...
$bundles = array(
    // ...
    new Draw\Bundle\NewRelicBundle\DrawNewRelicBundle(),
    // ...
);
```
...
draw_new_relic:
    ...
    monolog: 
        ...
        service: app.my_custom_handler    # Define a custom log handler (default: draw.new_relic.monolog_handler)
    commands: 
...
The config key`draw_new_relic.interactor` will accept a service ID to a service implementing `NewRelicInteractorInterface`. 
This bundle comes with a few services that may be suitable for you. 

| Configuration value | Description |
| ------------------- | ----------- |
| `Draw\Bundle\NewRelicBundle\NewRelic\AdaptiveInteractor` | This is the default interactor. It will check once per request if the NewRelic PHP extension is installed or not. It is a decorator for the `NewRelicInteractor` | 
| `Draw\Bundle\NewRelicBundle\NewRelic\NewRelicInteractor` | This interactor communicates with NewRelic. It is the one decorator that actually does some work. | 
| `Draw\Bundle\NewRelicBundle\NewRelic\BlackholeInteractor` | This interactor does nothing. | 
| `auto` | This value will check if the NewRelic PHP extension is installed when you build your container. | 

Note that if you set `draw_new_relic.enabled: false` you will always use the `BlackholeInteractor` no matter what value 
used for `draw_new_relic.interactor`.
...
This bundle is a fork of [webgriffe/EkinoNewRelicBundle](https://github.com/webgriffe/EkinoNewRelicBundle)
(originally `ekino/newrelic-bundle`) at version 2.5.0, re-namespaced under `Draw\Bundle\NewRelicBundle`
and updated for PHP 8.5 / Symfony 7.4. The original MIT license is preserved in `LICENSE`.
