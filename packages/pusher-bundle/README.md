...
`Draw\Bundle\PusherBundle\DrawPusherBundle` in your `config/bundles.php` file.
...
return [
    // ...
    Draw\Bundle\PusherBundle\DrawPusherBundle::class => ['all' => true],
    // ...
];
```
...
First, create an authorization service that implements `Draw\Bundle\PusherBundle\Authenticator\ChannelAuthenticatorInterface`:
...
use Draw\Bundle\PusherBundle\Authenticator\ChannelAuthenticatorInterface;
...
Then include its **service id** in the draw_pusher `auth_service_id` configuration
parameter:

```yml
# config/packagers/draw.pusher.yml
draw_pusher:
    # ...
...
# config\routing.yml
draw_pusher:
    resource: "@DrawPusherBundle/Resources/config/routing.xml"
    prefix:   /pusher
```
...
<script type="text/javascript">
    Pusher.channel_auth_endpoint = "{{ path('draw_pusher_bundle_auth') }}";
</script>
```

## Credits

This bundle is a fork of [laupiFrpar/LopiPusherBundle](https://github.com/laupiFrpar/LopiPusherBundle)
at version 6.1.0, re-namespaced under `Draw\Bundle\PusherBundle` and upgraded for PHP 8.5 /
Symfony 7.4 (the upstream Symfony 7 support request,
[issue #75](https://github.com/laupiFrpar/LopiPusherBundle/issues/75), is still open). The original
